...
from src.infraestructure.api_berfair.api import Api
...
#######################################################
file_real_actual_off = join(BASE_DIR, 'data', 'data_actual_off.json')
...
def get_realtime_data(settings: dict, callback: callable = _callable) -> None:
    ...
    trading = Trading(settings_real_time=settings, callback_real_time=callback)
    total = 100000
    sum_seg = 0
    # if session is True
    session = True
